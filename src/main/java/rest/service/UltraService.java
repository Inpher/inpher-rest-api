...
public class UltraService {
    ...
    public Response deleteSharingGroup(@FormParam("groupName") String groupName,
        ...
        try {
            sfs.deleteSharingGroup(groupName);
            return Response.status(201).entity("Sharing group successfully deleted.").build();
        } catch (NonExistingGroupException e) {
            log.error(e.getMessage(), e);
            return Response.status(400).entity("The group does not exist")
                    .build();
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            return Response.status(400).entity("A problem occurred, please try again later.")
    ...
    public Response uploadFile(@FormDataParam("content") final InputStream content,
        ...
        try {
            try {
                ...
                return Response.status(400)
                        .entity("An error occurred. Please check: " + e.getMessage()).build();
            }
            try {
                sfs.upload(file, FrontendPath.parse(name));
            } catch (ParentNotFoundException e) {
                log.error(e.getMessage(), e);
                return Response.status(400).entity("The parent of the dir does not exists.")
                        .build();
            } catch (InpherRuntimeException | IllegalArgumentException e) {
                log.error(e.getMessage(), e);
                return Response.status(400)
